...
// BindTable syncs a react.List to a QTableWidget. render returns column values for each item.
func BindTable[T any](list *react.List[T], table *qt.QTableWidget, render func(item T, index int) []string) {
	sync := func() {
		items := list.Items()
